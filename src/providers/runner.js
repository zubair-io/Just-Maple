...
try {
  ...
  if (!readiness.installed || !readiness.adapterInstalled || readiness.authenticated !== true) {
    ...
    process.stdout.write(JSON.stringify({ok:true,text:'Signed in; ready to test.'}));
  } else {
  ...
  process.stdout.write(JSON.stringify({ok:false,error:limited ? 'Provider subscription limit reached. Retry after your allowance resets.' : 'Provider could not complete this request. Check its login, subscription allowance and availability, then retry.'}));
} finally {
