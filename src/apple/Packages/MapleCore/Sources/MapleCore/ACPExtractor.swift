...
public struct ACPExtractor: FactExtractor, TaskCandidateExtractor {
    ...
    public static func tasks(_ text:String,event:Event,activities:[LifeActivity],provider:String)throws->[TaskSuggestion] {
        ...
        return try tasks.compactMap {c -> TaskSuggestion? in
            ...
            guard !c.title.trimmingCharacters(in:.whitespacesAndNewlines).isEmpty,c.title.count<=500 else {throw MapleError.provider("Task title must name the specific requested action and subject.")}
            guard !c.quote.isEmpty,event.content.contains(c.quote) else {throw MapleError.provider("Task quote is not an exact source passage.")}
            guard c.deadline.isEmpty || event.content.contains(c.deadline) else {throw MapleError.provider("Task deadline wording is not an exact source passage.")}
            guard Set(c.activityIDs).isSubset(of:Set(activities.map(\.id))) else {throw MapleError.provider("Task activity is not an allowed activity ID.")}
            var s=TaskSuggestion();s.eventID=event.id;s.quote=c.quote;s.provider="acp/\(provider)/tasks-v3-current-obligation";s.candidate.title=c.title;s.candidate.description=c.details ?? c.quote;s.candidate.activityIDs=c.activityIDs
